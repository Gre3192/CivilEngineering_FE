// src/components/steel/kit.jsx
// Primitivi UI condivisi per la materia "Acciaio" (accento slate).
...
export function ResultRow({ label, value, unit, strong, highlight }) {
    return (
        <div className={`flex items-baseline justify-between border-b border-slate-100 py-1 ${highlight ? "rounded-md bg-slate-100 px-2" : ""}`}>
...
export function BigResult({ title, value, unit, note }) {
    return (
        <div className="rounded-2xl border border-slate-300 bg-gradient-to-br from-slate-50 to-white p-5">
...
export function NumberField({ label, unit, value, onChange, step = "1" }) {
    return (
        <label className="block">
            ...
            <input type="number" value={value} step={step} onChange={(e) => onChange(e.target.value)}
                className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200" />
        </label>
    );
}
    ...
    return (
        <label className="block">
            ...
            <select value={value} onChange={(e) => onChange(e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200">
